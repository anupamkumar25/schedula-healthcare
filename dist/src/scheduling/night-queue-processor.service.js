...
let QueueProcessorService = class QueueProcessorService {
    ...
    async processNightQueue() {
        if (this.isProcessing) {
            console.log('Night queue processing is already running. Skipping this interval.');
            return;
        }
        try {
    ...
    async processDoctorQueue(doctorId, date) {
        ...
        const tree = await this.segmentTreeService.buildAndPopulateTree(doctorId, date, availability);
        if (!tree) {
        ...
        for (const queueEntry of queueEntries) {
            try {
                ...
                console.log('🔍 Finding optimal slot for:', doctorId, date, predictedDuration);
                ...
                await this.availabilityManagementRepo.createAppointmentFromQueue(queueEntry, appointmentTime, date);
